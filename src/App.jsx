...
import { 
  Activity, Trash2, Plus, Upload, Download
} from 'lucide-react';
...
const POSSIBLE_ONDERDELEN = ['Speed', 'Endurance', 'Freestyle', 'Double under', 'Triple under', 'DDC', 'DD4'];

export default function App() {
  const [view, setView] = useState('live'); 
  const [user, setUser] = useState(null);
  
  // Initiële state gebaseerd op originele 'ropeScore pro.txt'
  ...
  const deleteOnderdeel = async (id) => {
    ...
    let update = { onderdelen: newOnderdelen };
    if (status.activeOnderdeelId === id) {
      update.activeOnderdeelId = newOnderdelen[0]?.id || '';
      update.activeOnderdeel = newOnderdelen[0]?.naam || '';
    }
  ...
  const LiveView = () => (
    <div style={{ height: '100vh', backgroundColor: '#0f172a', color: '#fff', fontFamily: 'sans-serif', padding: '2rem', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      ...
      {/* ORIGINELE ONDERBALK (Speaker / Display Link) */}
      <div style={{ 
        marginTop: '2rem', 
        padding: '1rem', 
        backgroundColor: '#1e293b', 
        ...
        <button onClick={() => setView('admin')} style={{ background: 'none', border: 'none', color: '#64748b', fontWeight: 700, cursor: 'pointer' }}>BEHEER</button>
        <div style={{ color: '#334155' }}>|</div>
        <div style={{ color: '#64748b', fontWeight: 700 }}>DISPLAY MODE</div>
      </div>
    </div>
  );
    ...
    return (
      <div style={{ minHeight: '100vh', backgroundColor: '#f8fafc', fontFamily: 'sans-serif', color: '#1e293b' }}>
        <header style={{ backgroundColor: '#fff', borderBottom: '1px solid #e2e8f0', padding: '1rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 10 }}>
          ...
          <button onClick={() => setView('live')} style={{ backgroundColor: '#0f172a', color: '#fff', border: 'none', padding: '0.6rem 1.2rem', borderRadius: '0.6rem', fontWeight: 800, cursor: 'pointer' }}>TERUG NAAR LIVE</button>
        </header>
        ...
        <main style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: '2rem' }}>
          ...
          <section>
            <div style={{ backgroundColor: '#fff', padding: '1.5rem', borderRadius: '1rem', border: '1px solid #e2e8f0', marginBottom: '1.5rem' }}>
              <h3 style={{ margin: '0 0 1rem 0', fontSize: '0.8rem', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase' }}>Nieuw Onderdeel</h3>
            ...
            <div style={{ backgroundColor: '#fff', padding: '1.5rem', borderRadius: '1rem', border: '1px solid #e2e8f0' }}>
              <h3 style={{ margin: '0 0 1rem 0', fontSize: '0.8rem', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase' }}>Programma</h3>
                ...
                {status.onderdelen?.map(o => {
                  ...
                  return (
                    <div key={o.id} onClick={() => updateStatus({ activeOnderdeelId: o.id, activeOnderdeel: o.naam })} style={{ 
                      ...
                      <div style={{ fontWeight: 800 }}>{o.naam} <span style={{ fontSize: '0.7rem', color: '#94a3b8', marginLeft: '0.5rem' }}>({count})</span></div>
                      <button onClick={(e) => { e.stopPropagation(); deleteOnderdeel(o.id); }} style={{ background: 'none', border: 'none', color: '#cbd5e1', cursor: 'pointer' }}><Trash2 size={16}/></button>
                    </div>
                  );
                })}
              </div>
          ...
          <section>
            {currentOnderdeelObj && (
              <>
                <div style={{ backgroundColor: '#fff', padding: '1.5rem', borderRadius: '1rem', border: '1px solid #e2e8f0', marginBottom: '1.5rem' }}>
                   <h2 style={{ margin: '0 0 1rem 0', fontSize: '1.1rem', fontWeight: 900 }}>Deelnemers: {currentOnderdeelObj.naam}</h2>
                   <label style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '1rem', border: '2px dashed #e2e8f0', borderRadius: '1rem', cursor: 'pointer', fontWeight: 700, color: '#64748b' }}>
                     <Upload size={18}/> Upload CSV voor {currentOnderdeelObj.naam}
                ...
                <div style={{ backgroundColor: '#0f172a', padding: '1.5rem', borderRadius: '1.5rem', color: '#fff' }}>
                  <h3 style={{ margin: '0 0 1rem 0', fontSize: '0.7rem', fontWeight: 800, color: '#3b82f6', textTransform: 'uppercase' }}>Veld Toewijzing</h3>
                  ...
                  <button onClick={() => status.fields.forEach((_,i) => setField(i, ''))} style={{ width: '100%', marginTop: '1rem', padding: '0.8rem', borderRadius: '1rem', backgroundColor: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', border: 'none', fontWeight: 800, cursor: 'pointer' }}>MAAK VELDEN LEEG</button>
                </div>
              </>
            )}
          </section>
